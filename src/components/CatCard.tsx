  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        const thisCatVotes = allVotes.filter(
          (vote: any) => vote.image_id === cat.id
        );

        const calculatedScore = thisCatVotes.reduce(
          (acc: number, vote: any) => acc + (vote.value === 1 ? 1 : -1),
          0
        );
        ...
        const thisCatFavs = allFavs.filter(
          (fav: any) => fav.image_id === cat.id
        );
